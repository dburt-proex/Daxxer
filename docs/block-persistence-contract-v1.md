...
Status: proposed implementation contract for issue #11.

## Persistence authority

DaxxerOS Local is the source of truth. The desktop bridge accepts page `blocks`, `properties`, `views`, and `rows` and writes the full page metadata object into YAML frontmatter. The Markdown body is a derived searchable projection, not the structured block authority.
...
Page content using this contract carries:

```json
{"contentSchemaVersion": 1}
```

Absence of `contentSchemaVersion` means legacy version 0.

A page declaring a version greater than the current supported version must fail visibly and must not be rewritten by the migration layer.

## Legacy block shape

Version 0 blocks are preserved as the existing shape:
...
Nested blocks remain recursive under `children` arrays. Migration walks the full tree and preserves IDs and unknown fields at every level.

## Migration

`Daxxer.BlockSchema.migratePage(page)` performs deterministic migration:

1. Missing version is interpreted as v0.
2. Every existing block is cloned without dropping unknown fields.
...
6. `contentSchemaVersion` becomes `1` only after migration processing.
7. Invalid or unsupported future content returns explicit errors instead of being silently rewritten.

Migration is idempotent: migrating an already-normalized v1 page produces the same structured result.
...
- removes only `richText` and `contentSchemaVersion`;
...
Unknown block types are not deleted or coerced. They remain in the page tree and produce a visible `unknown_block_type` warning. Invalid rich-text values or unsupported future schema versions produce blocking errors.

This is intentional: preserving opaque user data is safer than guessing how to transform it.
...
Defining the contract and passing its migration tests resolves the serializer/migration gate. The editor may only begin writing v1 `richText` after the schema module is loaded in the app and persistence calls route through `prepareForPersistence()` or an equivalent validated boundary.
