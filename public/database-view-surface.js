...
(function () {
  ...
  Daxxer.Database.mount = function projectedMount(container, page, opts = {}) {
    ...
    const persist = () => { if (opts.onChange) opts.onChange(state); };
    ...
    function tableProjection(projection) {
      ...
      const rows=[...tbody.querySelectorAll("tr.db-row")]; rows.forEach(tr=>tr.hidden=!allowed.has(String(tr.dataset.row)));
      const add=tbody.querySelector(".db-add-row"); rows.filter(r=>!r.hidden).sort((a,b)=>order.get(String(a.dataset.row))-order.get(String(b.dataset.row))).forEach(r=>tbody.insertBefore(r,add));
    ...
    function editableTitle(row, className="dv-title-edit") { const div=document.createElement("div"); div.className=className; div.contentEditable="true"; div.textContent=title(row); div.onblur=()=>{const p=titleProp(); if(p){row.cells[p.id]=div.textContent.trim();persist();}}; div.onkeydown=e=>{if(e.key==="Enter"){e.preventDefault();div.blur();}}; return div; }
    ...
    function addView() { const type=(prompt(`View type: ${TYPES.join(", ")}`,"table")||"").trim();if(!TYPES.includes(type))return;const name=prompt("View name:",type[0].toUpperCase()+type.slice(1));if(!name)return;const v=Engine.createView(type,state,name);state.views.push(v);persist();host.innerHTML="";api=base(host,state,opts);observe();requestAnimationFrame(()=>{const tabs=[...host.querySelectorAll(".db-view-tab:not(.db-view-add)")];tabs[tabs.length-1]?.click();decorate();}); }
