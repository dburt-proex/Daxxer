...
export class EventSpine {
  ...
  async recordWorkspaceEvent(type, payload = {}, traceContext = {}) {
    await this.load();
    const correlationId = traceContext.correlationId || id();
    const directiveId = traceContext.directiveId || null;
  ...
  async scanGitHubRepository(repository, traceContext = {}) {
    await this.load();
    ...
    try {
      ...
      const receipt = {
        ...
        requestedAt: startedAt, completedAt: now(), directiveId, correlationId,
      ...
      const records = [];
      records.push(await this.recordEvent(this.makeEvent({ domain: "github", type: "github.repository_observed", actor: { kind: "adapter", id: "github-read-only" }, source, directiveId, correlationId, payload: {
      ...
      for (const entry of (Array.isArray(root) ? root : [])) {
        records.push(await this.recordEvent(this.makeEvent({ domain: "github", type: "github.root_entry_observed", actor: { kind: "adapter", id: "github-read-only" }, source: { ...source, resourceType: entry.type === "dir" ? "directory" : "file", locator: `https://github.com/${repository}/blob/${repo.default_branch}/${entry.path}` }, directiveId, correlationId, payload: { repository, path: limited(entry.path), entryType: limited(entry.type), size: Number(entry.size || 0) }})));
      }
      await this.append({ kind: "checkpoint", value: this.checkpoint("github_scan", { directiveId, correlationId }) });
      return { receipt, records, checkpoint: this.entries.at(-1).record.value };
    } catch (error) {
      const receipt = { schemaVersion: VERSION, id: id(), adapter: "github", mode: "read_only", operation: "read_repository", status: "failed", requestedAt: startedAt, completedAt: now(), directiveId, correlationId, request: { resource: `github://${repository}` }, result: { itemCount: 0, contentHash: hash({ repository, error: String(error?.message || error) }) }, warnings: [], error: { code: "github_read_failed", retryable: true } };
      await this.append({ kind: "adapter_receipt", value: receipt });
  ...
  makeEvent({ domain, type, actor, source, payload, directiveId = null, correlationId = id() }) {
    return { schemaVersion: VERSION, id: id(), occurredAt: now(), domain, type, actor, source, directiveId, correlationId, payload, contentFingerprint: hash({ source, payload }) };
  }
